...
int main(int argc, char **argv)
{
    ...
    auto cli                = with_prefixes_short_long(
        ...
        (option("help", "h").call([&]() { selected = mode::help; })) % "Show this message. \n You can use \'--filter\', \'--range\' or \'--wavelength\' to list options and use \'--format\' for more information of formatting output."
            | ((option("filter").call([&]() { selected |= mode::filterHelp; })
               ...
               option("com") & value("port number", conf.COM)));
    ...
    // control serial
    Eigen::Matrix<uint32_t, LED_LENGTH, LED_WIDTH> ar = Eigen::Matrix<uint32_t, LED_LENGTH, LED_WIDTH>::Zero();
    serial ser(conf.COM);
    ...
    try
    {
        ...
        size_t row = 0, col = 0;
        //std::atomic_int8_t row = 0, col = 0;
        uint32_t colorLog = white(0);
        std::thread t([&]() {
            if (!ser.send_ar(Eigen::Matrix<uint32_t, LED_LENGTH, LED_WIDTH>::Zero())) //! send a dark to meansure background
            {
                //?if not succeed than break and stop vege
                // or try again?
                device.stopRun();
                return 0;
            }
            ser.send_active();
            device.toldVegaDataisReadyandWait();

            //for (uint8_t color_mode = 1; !(color_mode & 1 << 7); color_mode <<= 1)
            uint8_t color_mode = colorMode::White;
            {
                for (size_t r = 0; r < ar.rows(); ++r)
                {
                    for (size_t c = 0; c < ar.cols(); ++c)
                        for (size_t bright : {255, 192, 128, 85, 64}) //1,3/4,1/2,1/3,1/4
                        {
                            uint32_t color = color_mode & colorMode::Red      ? rgb(bright, 0, 0)
                                             : color_mode & colorMode::Green  ? rgb(0, bright, 0)
                                             : color_mode & colorMode::Blue   ? rgb(0, 0, bright)
                                             : color_mode & colorMode::Yellow ? rgb(bright, bright, 0)
                                             : color_mode & colorMode::Purple ? rgb(bright, 0, bright)
                                             : color_mode & colorMode::Cyan   ? rgb(0, bright, bright)
                                             : color_mode & colorMode::White  ? rgb(bright, bright, bright)
                                                                              : rgb(0, 0, 0);
                            ...
                            //if send succeed than getdata and send next ar active
                            device.toldVegaDataisReadyandWait(); // wait for meansure
        ...
        t.detach();
        unsigned int dataNum = 0, dataIndex = 0;
        size_t rollbackSize = 0;
        boost::circular_buffer<VegaData> valuesbuf(5);
        struct singlePixelData
        {
            unsigned int index;
            unsigned int row;
            unsigned int col;
            ...
            double mean;
            double deviation;
            boost::circular_buffer<VegaData> details;
            singlePixelData(unsigned int i, unsigned int r, unsigned int c, uint32_t cc, double m, double de, const boost::circular_buffer<VegaData> &d)
                : index(i), row(r), col(c), color(cc), mean(m), deviation(de), details(d) {}
            singlePixelData()
                : index(0), row(0), col(0), color(0), mean(0), deviation(0), details(0) {}
        };
        device.registerDataReadyCallback([&](long hDevice, long channel) {
            ...
            if (ser.is_Active())
            {
                ...
                {
                    valuesbuf.push_back(VegaData(++dataIndex, values[i], timestamps[i], statuses[i]));
                }
                if (!valuesbuf.full()) return;
                double mean      = mean_v(valuesbuf);
                double deviation = deviation_m(valuesbuf, mean);
                if (deviation / mean > 0.01f)
                    return;

                //data.emplace_back(++dataNum, row, col, colorLog, mean, deviation, valuesbuf);
                singlePixelData data(dataNum, row, col, colorLog, mean, deviation, valuesbuf);
                valuesbuf.clear();
                device.toldVegaDataWasRead();
                std::stringstream output;
                std::string output_str;
                std::stringstream color_str;
                color_str.unsetf(std::stringstream::dec);
                color_str.setf(std::stringstream::hex);
                color_str.setf(std::stringstream::uppercase);
                color_str << data.color;
                output << data.index << '\t' << data.row << '\t' << data.col << '\t' << color_str.str() << '\t' << data.mean << '\t' << std::scientific << std::setprecision(4) << data.deviation;
                ...
                {
                    std::string str = std::string(rollbackSize, '\b') + std::to_string(dataNum) + " data received.";
                    std::cout << str;

                    rollbackSize = str.size();
                }
                else
                {
                    ...
                    //char buf[255];
                    //std::string format = std::to_string(dataNum) + '\t' + conf.format;
                    //std::strftime(buf, sizeof(buf), format.c_str(), localtime(&tt));
                    //std::string ret(buf);
                    //std::stringstream data;
                    //data << std::scientific << std::setprecision(4) << values[0];
